...
import { mockFarmer, mockActiveBooking } from '../services/mockData';
...
export const AuthProvider = ({ children }) => {
  ...
  const [farmer, setFarmer] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : mockFarmer;
  });
  const [activeBooking, setActiveBooking] = useState(() => {
    const saved = localStorage.getItem('active_booking');
    return saved ? JSON.parse(saved) : mockActiveBooking;
  });
  ...
  const login = async (phone, password) => {
    ...
    try {
      ...
      const role = res.role || (phone === '9999999999' ? 'ADMIN' : (phone === '9876543211' ? 'OWNER' : 'FARMER'));
      setUserRole(role);
