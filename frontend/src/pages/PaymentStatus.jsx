...
import { mockPaymentsHistory } from '../services/mockData';
...
export const PaymentStatus = () => {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setPayments(mockPaymentsHistory);
      }
    } catch (err) {
  ...
  const latest = payments[0] || mockPaymentsHistory[0];
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 relative text-left flex flex-col gap-4">
            ...
            <div className="bg-gradient-to-r from-emerald-800 to-teal-900 text-white p-4 rounded-2xl flex items-center justify-between">
              <div>
                <span className="text-[10px] text-emerald-200 font-extrabold uppercase block">TOTAL DISBURSED AMOUNT</span>
                <div className="text-2xl font-black text-amber-300 mt-0.5">₹{selectedReceipt.amount?.toLocaleString('en-IN')}</div>
              </div>
